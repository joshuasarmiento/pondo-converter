...
// 1. Get List of Anomalies (For the user to select from the UI dropdown)
app.get('/api/anomalies', authenticate, async (req, res) => {
    try {
        ...
        const anomalies = await prisma.anomaly.findMany({
            ...
            take: search ? 100 : 50 // Return up to 100 for search queries
        });
        res.json(anomalies);
    } catch (error) {
        console.error(error);
...
app.get('/api/convert/:anomalyId', authenticate, async (req, res) => {
    try {
        ...
        const conversions = latestCommodities.map(item => {
            // e.g., 50kg sack price = base_price (per kg) * 50
